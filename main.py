...
BLUE_BLOCK_IMAGE = pygame.image.load(os.path.join('Assets', 'blue_block.png'))
BLUE_BLOCK_IMAGE = pygame.transform.scale(BLUE_BLOCK_IMAGE, (55, 75))
RED_BLOCK_IMAGE = pygame.image.load(os.path.join('Assets', 'red_block.png'))
RED_BLOCK_IMAGE = pygame.transform.scale(RED_BLOCK_IMAGE, (55, 75))
RED_BLOCK_IMAGE = pygame.transform.flip(RED_BLOCK_IMAGE, True, False)
...
def draw_window(red, blue, red_health, blue_health, blue_is_punching, red_is_punching, blue_is_blocking, red_is_blocking, keys_pressed):
    WIN.blit(RING, (0, 0))
    ...
    elif blue_is_blocking:
        WIN.blit(BLUE_BLOCK_IMAGE, (blue.x, blue.y))
    else:
        WIN.blit(BLUE_BOXER, (blue.x, blue.y))

    if red_is_punching:
        WIN.blit(RED_PUNCH_IMAGE, (red.x, red.y))
    elif red_is_blocking:
        WIN.blit(RED_BLOCK_IMAGE, (red.x, red.y))
    else:
...
def main():
    ...
    blue_is_blocking = False  # Define it here
    red_is_blocking = False   # Define it here
    ...
    while run:
        ...
        if blue_is_punching and blue.colliderect(red):
            if not red_is_blocking:  # Only deduct health if Red isn't blocking
                red_health -= 1
        if red_is_punching and red.colliderect(blue):
            if not blue_is_blocking:  # Only deduct health if Blue isn't blocking
                blue_health -= 1
        ...
        # Handling Blue's Blocking
        if keys_pressed[pygame.K_g]:
            blue_is_blocking = True
        else:
            blue_is_blocking = False

        # Handling Red's Blocking
        ...
        WIN.fill(WHITE)
        draw_window(red, blue, red_health, blue_health, blue_is_punching, red_is_punching, blue_is_blocking, red_is_blocking, keys_pressed)
